...
class EmailService {
    ...
    async init() {
        try {
            ...
            // Skip verification entirely - it times out on Vercel
            // SMTP connection will be established on first actual email send
            console.log('✅ Email transporter created (skip verification on Vercel)');
            return true;
        } catch (error) {
    ...
    async sendCustomerEmail(customerData) {
        try {
            ...
            const mailOptions = {
                ...
                to: customerEmail,
                subject: `✨ Your Glam Session (${serviceSelected}) is Booked!`,
                html: html,
                replyTo: this.config.adminEmail
            };

            console.log(`📤 Sending customer email via SMTP (${this.config.host}:${this.config.port})...`);
            ...
            console.error('   SMTP Host:', this.config.host);
            console.error('   SMTP Port:', this.config.port);
            throw error;
        }
    }
    ...
    async sendAdminEmail(bookingData) {
        try {
            ...
            console.log(`📤 Sending admin email via SMTP (${this.config.host}:${this.config.port})...`);
            const result = await this.transporter.sendMail(mailOptions);
